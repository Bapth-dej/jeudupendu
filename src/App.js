...
class App extends Component {
  ...
  startGame = () => {
    this.setState({
      playing: true,
      wordArray:
        EXPRESSIONS_ARRAY[Math.floor(Math.random() * EXPRESSIONS_ARRAY.length)],
      lettersClicked: [" "]
    });
  };
  ...
  handleLetterClick = letter => {
    ...
    if (!letterDiscovered) {
      this.setState(prevState => ({
        lettersClicked: [...lettersClicked, letter]
      }));
    }
  };
  ...
  wordWithHiddenLetter() {
    ...
    return wordArray.map((letter, index) => (
      <span className={"wordLetter"} key={index}>
        {lettersClicked.includes(letter) ? (
  ...
  hasWon() {
    ...
    return !wordArray
      .map(letter => (lettersClicked.includes(letter) ? letter : "_"))
      .includes("_");
  }
          ...
          {playing ? (
            <div className="playing">
              {this.hasWon() ? (
                ...
                <h2>
                  Trouvez le(s) mot(s) ci - dessous, en
                  {
                    wordArray.filter(function(value) {
                      return value !== " ";
                    }).length
                  }
                  lettres
              ...
              </div>
              {
                <div className="guesses">
                  Essais ratés:
                  {lettersClicked.map((letter, index) => (
                    <span className="symbol" key={index}>
                      {wordArray.includes(letter) ? "" : letter}
                    </span>
                  ))}
                </div>
              }
